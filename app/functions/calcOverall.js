...
function calcOverall(stats, MOEstats) {
    ...
    stats.map((row) => {
        if (tankNames[row.tank_id]) {
            ...
            let vehicleStats = {
                id: row.tank_id,
                name: tankNames[row.tank_id]['short_name'] + (tankNames[row.tank_id]['is_premium'] ? " 🟊" : ""),
                nation: nationConversion[tankNames[row.tank_id]['nation']],
                ...
                isPrem: tankNames[row.tank_id]['is_premium']
            };
